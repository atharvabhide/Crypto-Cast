...
def get_ltc():
    ...
    ltc = ltc.drop(['Open', 'High', 'Low', 'Close', 'Volume'], axis=1)

    # Load LSTM model
    lstm_model = load_model(os.path.join(os.getcwd(), "lstm", "models", "LTC-CAD.h5"))
    scaler = MinMaxScaler(feature_range=(0, 1))
    scaled_data = scaler.fit_transform(ltc['Adj Close'].values.reshape(-1, 1))
    ...
    ax = ltc['2018':].plot(label='Observed (SARIMAX)')
    ...
    ax = ltc.plot(label='Observed (SARIMAX)', color='grey', figsize=(20, 8))
    ...
    ltc['Adj Close'].plot(label='Observed (LSTM)', color='blue')
    future_index = pd.date_range(start=ltc.index[-1], periods=366, closed='right')  # One extra day for leap year
    ...
    plt.title('Litecoin (LTC) Forecasting (LSTM)')
    plt.xlabel('Date')
    ...
    fig = px.line(x=One_week_values_sarimax.index, y=One_week_values_sarimax.values,
                      labels={'x': 'Date', 'y': 'CAD Dollars'}, title="Litecoin (LTC) forecasting (SARIMAX) - One Week",
                      markers=True)
    ...
    fig = px.line(x=pred_uc.predicted_mean.index, y=pred_uc.predicted_mean.values,
                  labels={'x': 'Date', 'y': 'CAD Dollars'}, title="Litecoin (LTC) forecasting (SARIMAX) - One Month",
                  markers=True)
    ...
    fig = px.line(x=future_index[:7], y=future_predictions_lstm[:7].flatten(),
                      labels={'x': 'Date', 'y': 'CAD Dollars'}, title="Litecoin (LTC) forecasting (LSTM) - One Week",
                      markers=True)
    fig.update_traces(line_color='#76D714', line_width=5)  # 00ff00
    with st.expander("👁 ", True):
            st.plotly_chart(fig, use_container_width=True)
    ...
    fig = px.line(x=future_index, y=future_predictions_lstm.flatten(),
                  labels={'x': 'Date', 'y': 'CAD Dollars'}, title="Litecoin (LTC) forecasting (LSTM) - One Month",
                  markers=True)
    fig.update_traces(line_color='#76D714', line_width=5)
    with st.expander("👁 ", True):
        st.plotly_chart(fig, use_container_width=True)
    ...
    fig = px.line(x=future_index, y=future_predictions_lstm.flatten(),
                  labels={'x': 'Date', 'y': 'CAD Dollars'}, title="Litecoin (LTC) forecasting (LSTM) - One Year",
                  markers=True)
    fig.update_traces(line_color='#76D714', line_width=5)
    with st.expander("👁 ", True):
        st.plotly_chart(fig, use_container_width=True)